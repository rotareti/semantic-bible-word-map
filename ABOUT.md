...
3. **Continuous Vector Training (Paradigmatic 50-Word Window)**: Using Gensim's Word2Vec Skip-gram architecture with negative sampling, the system trains 100-dimensional dense semantic vectors using a 50-word context envelope. While a small 5-word window captures immediate local syntax (*in praesentia*, e.g. titular collocations like *King Sennacherib* or *Eleazar the priest*), the expanded 50-word window covers the entire verse envelope (spanning 98% of all biblical verses), capturing rich **paradigmatic and thematic semantics** (*in absentia*). This shifts the semantic topology toward institutional functions, covenantal structures, and doctrinal relationships (e.g. *priest* connecting to *consecrated*, *priesthood*, *atonement*, and *Levites*; *covenant* connecting to *commandments*, *treaty*, *tablet*, *guarantee*, and *mediator*).
