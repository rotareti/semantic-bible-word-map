...
### Fixed
- **Autocomplete Sequence Initialization:** Fixed an issue where the autocomplete sequence counter was uninitialized on initial page load, causing arithmetic operations to evaluate to NaN. This caused the sequence check to falsely treat initial search debounce timers as obsolete, aborting centroid calculations and leaving the search loading spinner active indefinitely until the search input was cleared or view mode switched.
